...
#if defined(NDEBUG)
#error "HodlCash cannot be compiled without assertions."
#endif
...
/** Fees smaller than this  are considered zero fee (for relaying and mining)
 * We are ~100 times smaller then bitcoin now (2015-06-23), set minRelayTxFee only 10 times higher
 * so it's still 10 times lower comparing to bitcoin.
 */
CFeeRate minRelayTxFee = CFeeRate(50000);
...
bool ContextualCheckCoinSpend(const CoinSpend& spend, CBlockIndex* pindex, const uint256& txid)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
            return error("%s : zHDLC with serial %s is already in the block %d\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zHDLC spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: zHDLC spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
{
    int64_t nSubsidy = 10000000;

    if (Params().NetworkID() == CBaseChainParams::TESTNET) {     //TESTNET
        if (nHeight < 2000000 && nHeight > 0)                    //TESTNET
            return 25 * COIN;                                  //TESTNET
    }                                                            //TESTNET
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from HodlCash
...
{
    RenameThread("HodlCash-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZHDLCMinted()
{
...
void RecalculateZHDLCSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zHDLC supply
...
bool RecalculateHDLCSupply(int nHeightStart)
{
...
bool UpdateZHDLCSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateHDLCSupply(Params().Zerocoin_StartHeight());
    }

    //Track zHDLC money supply in the block index
    if (!UpdateZHDLCSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zHDLC supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zHDLCSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert HDLC to zHDLC
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zHDLC spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zHDLC serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zHDLC mints and %d zHDLC spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // HodlCash: We use certain sporks during IBD, so check to see if they are
