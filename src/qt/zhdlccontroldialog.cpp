...
#include "ui_zhdlccontroldialog.h"
...
std::list<std::string> ZHDLCControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZHDLCControlDialog::listMints;

ZHDLCControlDialog::ZHDLCControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZHDLCControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZHDLCControlDialog::~ZHDLCControlDialog()
{
    delete ui;
}

void ZHDLCControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZHDLCControlDialog::updateList()
{
...
void ZHDLCControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZHDLCControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZHDLC_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZHDLCControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZHDLCControlDialog::GetSelectedMints()
{
...
void ZHDLCControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
