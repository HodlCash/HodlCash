...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::HDLC)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sHDLCPercentage, QString& szHDLCPercentage)
{
    int nPrecision = 2;
    ...
    szHDLCPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sHDLCPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
   ...
   CAmount hdlcAvailableBalance = balance - immatureBalance - nLockedBalance;
   CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;

    // HDLC Watch-Only Balance
    ...
    // Combined balances
    CAmount availableTotalBalance = hdlcAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // HDLC labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, hdlcAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelHDLCPercent->setText(sPercentage);
    ui->labelzHDLCPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zHDLC.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", true);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in HodlCash.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in HodlCash.conf");
    }
    ...
    bool showHDLCAvailable = settingShowAllBalances || hdlcAvailableBalance != nTotalBalance;
    bool showWatchOnlyHDLCAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showHDLCPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyHDLCPending = watchUnconfBalance != 0;
    bool showHDLCLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyHDLCLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showHDLCAvailable || showWatchOnlyHDLCAvailable);
    ui->labelBalanceText->setVisible(showHDLCAvailable || showWatchOnlyHDLCAvailable);
    ui->labelWatchAvailable->setVisible(showHDLCAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showHDLCPending || showWatchOnlyHDLCPending);
    ui->labelPendingText->setVisible(showHDLCPending || showWatchOnlyHDLCPending);
    ui->labelWatchPending->setVisible(showHDLCPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showHDLCLocked || showWatchOnlyHDLCLocked);
    ui->labelLockedBalanceText->setVisible(showHDLCLocked || showWatchOnlyHDLCLocked);
    ui->labelWatchLocked->setVisible(showHDLCLocked && showWatchOnly);
    ...
    bool showzHDLCAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzHDLCUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzHDLCImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzHDLCAvailable);
    ui->labelzBalanceMatureText->setVisible(showzHDLCAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzHDLCUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzHDLCUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzHDLCImmature);
    ui->labelzBalanceImmatureText->setVisible(showzHDLCImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelHDLCPercent->setVisible(showPercentages);
    ui->labelzHDLCPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("HDLC")
    updateDisplayUnit();
}
