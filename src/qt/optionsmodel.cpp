...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::HDLC);
    ...
    if (!settings.contains("nAnonymizeHodlCashAmount"))
        settings.setValue("nAnonymizeHodlCashAmount", 1000);

    nAnonymizeHodlCashAmount = settings.value("nAnonymizeHodlCashAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeHodlCashAmount"))
        SoftSetArg("-anonymizeHodlCashamount", settings.value("nAnonymizeHodlCashAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in HodlCash.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeHodlCashAmount:
            return QVariant(nAnonymizeHodlCashAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeHodlCashAmount:
            nAnonymizeHodlCashAmount = value.toInt();
            settings.setValue("nAnonymizeHodlCashAmount", nAnonymizeHodlCashAmount);
            emit anonymizeHodlCashAmountChanged(nAnonymizeHodlCashAmount);
            break;
        case CoinControlFeatures:
